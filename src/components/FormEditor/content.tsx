...
interface IField<T extends keyof IEditorField> {
  field: T;
}
...
export default function EditPage({ fetch, fields, data, ...props }: { data: any, fields: IEditorField[], fetch: Function, }) {
  const local = useLocalObservable<{ jsonMap: { [key: string]: string }, attributes: { field: keyof IEditorField, name: string, value: any, disabled?: boolean }[], temp: IEditorField | null }>(() => ({
    ...
    attributes: [
      ...
      { field: 'field', name: '字段', value: '', disabled: true },
      { field: 'component', name: '控件', value: '' },
    ],
    temp: null
  }))
  useEffect(() => {
  ...
  return <Observer>{() => (<Fragment>
    <Row>
            ...
            switch (item.component) {
              case 'Input':
                return <Form.Item key={item.field} label={item.title} labelCol={lb} wrapperCol={rb}>
                  <Input defaultValue={item.defaultValue as string} autoFocus={item.autoFocus || false} onChange={e => {
      ...
      {local.temp && (
        <Col >
          <div>{local.attributes.map((attr) => (
            <Input key={attr.field} addonBefore={attr.name} value={local.temp ? (local.temp[attr.field] as string) : ''} />
          ))}</div>
        </Col>
      )}
    </Row>
